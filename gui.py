...
class CMainGUI():
    ...
    def loginConnect(self):
        ...
        self.pw = md5.md5(str(self.login.passwordEdit.text())).hexdigest()
        ...
        self.window.statusLbl.setText("<font style='color: red;background: black;'>STATUS: Not injected</font>")
        self.log("::LOG STARTED::")
        self.window.show()
        self.window.listWidgetQuest.customContextMenuRequested.connect(self.openQuestContexMenu)
        self.window.listWidgetQuest2.customContextMenuRequested.connect(self.openQuestContexMenu2)
        ...
        self.window.listWidget.customContextMenuRequested.connect(self.openLoggerContexMenu)
        self.sendDlg.sendBtn.clicked.connect(self.send)

        self.sendDlg.listBlocker.customContextMenuRequested.connect(self.openBlockListContexMenu)
        ...
        self.settingsDlg.connect(self.settingsDlg.opacitySlider, QtCore.SIGNAL('valueChanged(int)'),self.changeOpacity)
    ...
    def openBlockListContexMenu(self,position):
        ...
        self.ctMenuQuest.exec_(self.sendDlg.listBlocker.viewport().mapToGlobal(position))
    ...
    def startBlocking(self):
        ...
        for i in range(0,self.sendDlg.listBlocker.topLevelItemCount()):
            ...
            stackGui.put("MSG_BLOCK|"+item.text(1)+"|"+item.text(2)+item.text(3))
    ...
    def handleQueue(self):
        while self.queueStarted:
            ...
            if(sMsg=="MSG_INJECTED"):
                ...
                self.window.statusLbl.setText("<font style='color: black;background: green;'>STATUS: Injected, have fun</font>") 
                ...
                self.log("Injection failed! Game not running or not started as administrator")     
                ...
                if(self.window.chkSend.isChecked()):
                    ...
                    for line in file: 
                        ids = line.split('|')
                        if(sSig==ids[0]):
                            self.addPacketItem(sPacket[1],"[C->S]",ids[1],50,205,50)
                            bUnknown = False
                            break
                    file.close()
                    if(bUnknown):
                        self.addPacketItem(sPacket[1],"[C->S]","ID_UNKNOWN",8,205,8)
                ...
                if(self.window.chkRecv.isChecked()):
                    ...
                    for line in file: 
                        ids = line.split('|')
                        if(sSig==ids[0]):
                            self.addPacketItem(sPacket[1],"[S->C]",ids[1],165,42,42)
                            bUnknown = False
                            break
                    file.close()
                    if(bUnknown):
                        self.addPacketItem(sPacket[1],"[S->C]","ID_UNKNOWN",165,00,00)
                    ...
                    for line in fobj: 
                        ...
                        if(sPacket!=""):
                            if(id[0]==str(int(sPacket,16))):
                                ...
                                item.setText(2,name)
                                item.setText(3,id[2])
                                self.window.listWidgetQuest2.insertTopLevelItem(i,item)
                                i+=1
                    fobj.close()
                else:
                    self.quit()
                    ...
                    if(sAction=="03"):
                        for line in fobj: 
                            ...
                            if(id[0]==str(nQuestId)):
                                ...
                                item.setText(2,name)
                                item.setText(3,id[2])
                                self.window.listWidgetQuest2.insertTopLevelItem(0,item)
                                break
                    elif(sAction=="04"):
                        pass
                    else:
                        list = self.window.listWidgetQuest2.findItems(str(nQuestId), QtCore.Qt.MatchExactly,0)
                        try:
                            b = list[0]
                            a = self.window.listWidgetQuest2.indexFromItem(list[0])
                            c = a.row()
                            b = self.window.listWidgetQuest2.takeTopLevelItem(c)
                        except IndexError:
                            print "no quest"
    ...
    def openQuestContexMenu(self,position):
        ...
        self.ctMenuQuest.exec_(self.window.listWidgetQuest.viewport().mapToGlobal(position))
    ...
    def openQuestContexMenu2(self,position):
        ...
        self.ctMenuQuest.exec_(self.window.listWidgetQuest2.viewport().mapToGlobal(position))
    ...
    def openLoggerContexMenu(self,position):
        ...
        self.ctMenuQuest.exec_(self.window.listWidget.viewport().mapToGlobal(position))
